...
export default defineComponent({
    ...
    prcpAnomalyKPI(): null|string {
      ...
      if (this.normals) {
        ...
        times.forEach((time, index) => {
          ...
          const date = parseISO(String(time))
          const normal = normals[date.getMonth()]?.prcp
          if (prcp !== null && normal) {
  ...
  methods: {
    ...
    async fetchData(): Promise<void> {
      ...
      if (this.station) {
        url += `stations/monthly?station=${this.station}`
      } else {
        url += `point/monthly?lat=${this.lat}&lon=${this.lon}&alt=${this.alt}`
      }
