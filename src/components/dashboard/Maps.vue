<template>
  <div ref="wrapper" class="wrapper">
    <div class="d-flex align-items-center">
      ...
      <div v-if="cells.length === 0 && !loading" class="ms-2">
      ...
      <div class="d-flex align-items-center ms-auto">
        <button type="button" class="btn btn-light d-none d-md-inline-block bg-white" @click="downloadMap()">
          <icon :icon="['fas', 'download']" />
        </button>
        <button type="button" class="btn btn-light d-none d-md-inline-block bg-white ms-1" @click="toggleFullscreen()">
        ...
        <div class="dropdown ms-2">
          ...
          <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
            <li>
              <a class="dropdown-item" @click="setParameter('tavg')">{{ t('$params.tavg') }}</a>
            </li>
            <li>
              <a class="dropdown-item" @click="setParameter('tmin')">{{ t('$params.tmin') }}</a>
            </li>
            <li>
              <a class="dropdown-item" @click="setParameter('tmax')">{{ t('$params.tmax') }}</a>
            </li>
            <li>
              <a class="dropdown-item" @click="setParameter('prcp')">{{ t('$params.prcp') }}</a>
            </li>
            <li>
              <a class="dropdown-item" @click="setParameter('wspd')">{{ t('$params.wspd') }}</a>
            </li>
            <li>
              <a class="dropdown-item" @click="setParameter('pres')">{{ t('$params.pres') }}</a>
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div ref="map" class="map my-2 rounded" />
    <div class="d-flex align-items-center">
      <div class="btn-group btn-group-sm rounded border">
        ...
        <button
          ...
          @click="play()"
        >
          <icon v-if="!playing" :icon="['fas', 'play']" />
          <icon v-if="playing" :icon="['fas', 'pause']" />
        </button>
        <button
          type="button"
          class="btn btn-light bg-white px-3"
      ...
      <small class="ms-auto">
        <a href="https://www.openstreetmap.org/" target="_blank" rel="noreferrer nofollow" class="text-muted"> OSM, </a>
        <a href="https://carto.com/attribution" target="_blank" rel="noreferrer nofollow" class="text-muted"> CARTO </a>
      </small>
    </div>
    <div class="legend mt-4">
      <div class="d-flex align-item-center">
        <div v-if="scale.zero" class="legend-item" :style="`background: ${scale.zero}`" />
        <div v-for="color in scale.colors" :key="color" class="legend-item" :style="`background: ${color}`" />
      </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    async fetchData(): Promise<void> {
      ...
      await parse(
        `${this.$api}/app/spatial?parameter=${this.parameter}&date=${format(
          this.date,
          'yyyy-MM-dd'
        )}&bounds=${bounds.join()}`,
        {
